...
class Sensor_Model extends CI_Model {
    ...
    public function updateMapInfo($pid,$mapId){
        $updated_data=array(
            'RegionID'  => $mapId
        );
    ...
    public function GetGroupId($user){
        $sql="select ID,GroupName from productgroup where UserID='".$user."' and isdelete=0 ORDER BY SortID";
        $query = $this->db->query($sql);
        return $query->result_array();
    }
